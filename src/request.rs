...
impl Request {
    ...
    pub fn validate_port(port: &str, valid_range: &str, allow_all: bool) -> bool {
        ...
        return Request::validate(port, valid_range, &validate_message);
    }
//@classmethod
...
mod tests {
    ...
    #[test]
    fn test_validate() {
        assert!(Request::validate("A", constant::VALID_DEVICES, "Invalid Message"));
        assert!(Request::validate("1", constant::VALID_TAIL_PORTS, "Invalid Message"));
        assert!(Request::validate("all", constant::VALID_TAIL_PORTS, "Invalid Message"));
        assert!(Request::validate("All", constant::VALID_TAIL_PORTS, "Invalid Message"));

        assert!(Request::validate_port("1", constant::VALID_TAIL_PORTS, true));
        assert!(Request::validate_port("all", constant::VALID_TAIL_PORTS, true));
        assert!(Request::validate_port("All", constant::VALID_TAIL_PORTS, true));

        let port = 1;
        assert!(Request::validate_port(&1.to_string(), constant::VALID_TAIL_PORTS, true));
    ...
    fn test_validate_panic() {
        assert!(!Request::validate("X", constant::VALID_DEVICES, "Invalid Message"));
    }

    #[test]
    #[should_panic(expected = "Invalid Port: 9")]
    fn test_validate_port_allow_all_panic() {
        assert!(!Request::validate_port("9", constant::VALID_SERVO_PORTS, true));
    }

    #[test]
    #[should_panic(expected = "Invalid Port: J")]
    fn test_validate_port_panic() {
        assert!(!Request::validate_port("J", constant::VALID_SERVO_PORTS, false));
    }
